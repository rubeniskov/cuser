...
export const MessageWriter = ({ topicId, className }) => {
  ...
  const handlePublish = useCallback((_) => publishMessage({ content: value, accessToken }), [accessToken, value]);
  const handleKeyDown = useCallback((evt) => evt.which === 13 && handlePublish(evt), [handlePublish]);
  const handleLogin = useCallback((_, payload) => authenticate(payload), [accessToken, value]);
  ...
  return (
    ...
    {accessToken ?
    <div>
      <ListItem side={<Avatar avatar={avatar} />}>
